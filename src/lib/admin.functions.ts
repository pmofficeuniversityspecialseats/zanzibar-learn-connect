...
export const getAdminOverview = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    if (!role) throw new Response("Huna ruhusa ya usimamizi.", { status: 403 });
    ...
    return { newSubmissions: submissions.count ?? 0, pendingContent: content.count ?? 0, recentSubmissions: recent.data ?? [] };
  });